...
class ConvertImageSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.transform is None:
            im_input = self.Transformer(im_input)
    ...
    def sample_loader(self, im_name):
        ...
        #
        # elif self.data_name == 'm3fd':
        #     return BSDgray_loader(self.dataroot, im_name)

        if self.data_name == 'M3FD_Detection':
            return M3FD_Detection(self.dataroot, im_name)

        elif self.data_name == 'M3FD_Detection_MiniSet':
            # return BSDgray_loader(self.dataroot, im_name)
            return M3FD_Detection_MiniSet(self.dataroot, im_name)

        elif self.data_name == 'M3FD_Detection_test_code':
            # return BSDgray_loader(self.dataroot, im_name)
            return M3FD_Detection_test_code(self.dataroot, im_name)

        elif self.data_name == 'M3FD_Detection_Other_Data':
            return M3FD_Detection_Other_Data(self.dataroot, im_name)
            ...
            # print("Unknown dataset.")原本else分支的设置
            # quit()

            
    def Transformer(self, im_input):
        # if self.data_name == 'RESIDE':
...
def BSDgray_loader(dataroot, im_name):    
    im_pth = dataroot+im_name+'.png'
    label  = Image.open(im_pth).convert('RGB')
    noisy  = Image.open(im_pth).convert('RGB') # to which noise will be added (in test).
    return noisy, label

def M3FD_Detection_MiniSet(dataroot, im_name):
    im_pth = dataroot+im_name+'.png'
    noisy  = Image.open(im_pth).convert('RGB') # to which noise will be added (in test).
    return noisy

def M3FD_Detection(dataroot, im_name):
    im_pth = dataroot+im_name+'.png'
    # noisy  = Image.open(im_pth).convert('RGB') # to which noise will be added (in test).
    noisy  = Image.open(im_pth) # to which noise will be added (in test).
    return noisy

def M3FD_Detection_Other_Data(dataroot, im_name):
    # im_pth = dataroot+im_name+'.png'
    # im_pth = dataroot+im_name
    im_pth = os.path.join(dataroot, im_name)
    noisy  = Image.open(im_pth).convert('RGB') # to which noise will be added (in test).
    return noisy

def M3FD_Detection_test_code(dataroot, im_name):
    im_pth = dataroot+im_name+'.png'
    noisy  = Image.open(im_pth).convert('RGB') # to which noise will be added (in test).
    return noisy
